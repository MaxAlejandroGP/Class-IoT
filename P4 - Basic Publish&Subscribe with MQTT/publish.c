...
void sensado(){
    ...
    //char data[20];
    sensor();
    /*sprintf(data, "%f", TEMP_FINAL);
    printf("%s",data);*/

    char buffer[20];
    snprintf(buffer, sizeof(buffer), "%.2f", TEMP_FINAL);
    const char* value1 = buffer;
    const char* payload = "%s";
    size_t payload_size = snprintf(NULL, 0, payload, value1) + 1;
    char* payload_buffer = malloc(payload_size);
    snprintf(payload_buffer,payload_size,payload,value1);

    int msg_id;
    msg_id = esp_mqtt_client_publish(client, "sensores/sensorTemp", payload_buffer, 0, 1, 0);
    ESP_LOGI(TAG, "sent publish successful, msg_ID=%d", msg_id);
    /* ------ Obtención de sensado -----*/
